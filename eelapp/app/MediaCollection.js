class MediaCollection extends React.Component {
	...
	handleKeyboard(e) {
		...
		if (e.keyCode === 78) {
			...
			while (this.lastRandomCards.indexOf(random) !== -1) random = this.randomInt(0, count - 1);
	...
	isInsideView(el, partially = true) {
		...
		let isVisible = elemTop >= 0 && elemBottom <= window.innerHeight && elemRight <= window.innerWidth;
	...
	toggleLove(id, e, offsetTop = false) {
		let scrollOffset = offsetTop ? offsetTop : e.target.parentElement.parentElement.offsetTop;
	...
	render() {
		...
		let posts = chosenData.map((data) => {
			...
			return (
				<div>
					<hr />
					<div className="columns is-mobile is-gapless" ref={(card) => this.cardsRef.push(card)} id={data.id}>
						...
						<div className="column" style={{ maxWidth: '40px' }}>
							<a onClick={(e) => this.toggleLove(data.id, e)} className={buttonLoveClass}>
								<img
							...
							<a onClick={(e) => this.moveUp(data.id, e)} className="button card-footer-item">
								<img
							...
							<a onClick={(e) => this.moveDown(data.id, e)} className="button card-footer-item">
								<img
							...
							<a onClick={(e) => this.toggleIgnore(data.id, e)} className="button card-footer-item">
								<img
									src="/app/img/ban.svg"
									alt="Ignore"
									style={{
		...
		if (!this.state.showLove && !this.state.showIgnore) buttonNormalClass = 'button is-warning';
			...
			<section className="section">
				<nav className="level navbar is-fixed-top">
					<div className="level-item">
						...
						<a ref={(i) => (this.buttonNormal = i)} className={buttonNormalClass} onClick={this.showNormal}>
							<span className="icon">
								<img
						...
						<a ref={(i) => (this.buttonLove = i)} className={buttonLoveClass} onClick={this.toggleShowLove}>
							<span className="icon">
								<img
									src="/app/img/heart.svg"
									alt="Love"
